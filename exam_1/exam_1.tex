...
\paragraph{Hypotheses} $H_0$: The data are drawn from a normal distribution. $H_a$: The data are not drawn froma normal disribution. Large p-value means normal.


\subsection*{Contrasts}
\hrule width 5cm
\vspace{6pt}
$\Gamma = \sum{i=1}^{a} c_i \mu_i$, where $c_i$'s are constants such that $\sum{i=1}^{a} c_i = 0$. To estimate $\Gamma$ use $\hat{\Gamma} = \sum{i=1}^{a}c_i \bar{y}_{i \cdot}$ \\
Hypotheses: $H_0$: $\sum{i=1}^{a} c_i \mu_i = 0$. $H_a$: $\sum{i=1}^{a} c_i \mu_i \neq 0$. (or $<$ or $>$). \\
Test statistic: $t = (\sum{i=1}^{a} c_i \bar{y}_{i \cdot}) / (\sqrt{\frac{MSE}{n}\sum{i=1}^{a} c_i^2})$. Reject $H_0$ if $t$ in C.R. of $t_{\alpha, N-a}$ (or $\alpha/2$ for 2-tail) or if p-value $< \alpha$. \\
Note: $F = (\sum{i=1}^{a} c_i \bar{y}_{i \cdot})^2 / (\frac{MSE}{n}\sum{i=1}^{a} c_i^2)$ can be used to test 2-tailed. Reject $H_0$ if $F > F_{\alpha,1,N-a}$ or if p-value $< \alpha$.
